#!/bin/bash
# Source: https://github.com/keylase/nvidia-patch/blob/master/patch.sh
...
while getopts 'rshc:ld:f' flag; do
    ...
    c)
        opmode="${opmode}checkversion"
        checked_version="$OPTARG"
        ;;
...
patch_common() {
    ...
    [[ "$dir_found" ]] || {
        echo "ERROR: cannot detect driver directory"
        exit 1
    }
